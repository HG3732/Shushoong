...
//        System.out.println("유저 등급 : " + authentication.getAuthorities());
	...
	public String hotelListSort(Model model, String loccode, String roomCap, String keyword, String maxPrice,
	...
	public String hotelPriceSort(Model model, HttpSession session, String loccode, String roomCap, String keyword,
			String maxPrice, String sortBy, String sortTo) {
		List<HotelDtoRes> result = service.selectAllHotelList(loccode, roomCap, keyword, maxPrice, sortBy, sortTo);
		for (HotelDtoRes dto : result) {
			dto.setPriceDiscounted(Integer.parseInt(dto.getHotelPrice()) * (100 - dto.getRoomDiscount()) / 100);
		}
	...
	public String hotelview(Model model, HttpSession session, @PathVariable String hotelCode) {
		// main 에서 session 안에 checkIn, checkOut 정보 들고들어와서 그 정보를 밑에서 model 안에 넣고 html
		// 페이지에 뿌림
		...
		// session에 담긴 정보는 string 으로 뽑을 수 있기 때문에 위에서 integer로 session에 넣었기 때문에 session
		// 에서 꺼낼 때 string으로 뽑아주고 그 다음에 integer로 다시 변환해주기
	...
	public String hotelPaging(Model model, String hotelCode, String currentPage) {
...
//		누른 현재 페이지 알아야함(어떻게 기준으로 삼을지..)
		...
		Map<String, Object> reviewDetailDto = service.selectReviewDetailList(hotelCode, reviewNum, reviewPageNum,
				currentPageNum);
		...
		// GsonBuilder().setPrettyPrinting().create().toJson(reviewDetailDto));
	...
	public String hotelPay(HttpSession session, Model model, String hotel, String hotelCode, String hotelLocCat,
		...
		model.addAttribute("checkIn", session.getAttribute("checkIn"));
		// 이거는 내가 정해놓은 이름에 session값을 넣는것이기 때문에 이 이름에 대한 자료형이 정해진게 없어서 session이 어떤 자료형이든
		// 그냥 들어갈 수 있음
		model.addAttribute("checkOut", session.getAttribute("checkOut"));
	...
	public Map<String, Object> reserveInfo(@RequestBody HotelReserveDtoRes reserverInfo,
...
//	결제 정보 저장
	...
	@ResponseBody
	public String hotelPayment(HttpSession session,
			// 이렇게 요청파라미터에 적으면 함수 내에서 새로 new해서 객체를 만들지 않아도 됨
			@RequestBody HotelReserveCompleteDtoRes reserveCompletedto) throws IOException, InterruptedException {

		// HotelReserveCompleteDtoRes reserveCompletedto = new
		// HotelReserveCompleteDtoRes();
		...
		// else if 하면 else로 인해서 하나 걸리면 참이 되는 조건인데 참이 되면 해당 조건에 맞는 코드 블록이 실행되고 나머지 조건들은
		// 평가되지 않아서 더해야 할 값이 있음에도 불구하고 더하지 않음
...
//		ajax로 보내지는 데이터 () 안에 작성
		HttpRequest request = HttpRequest.newBuilder()
				...
				// payments/" + 결제번호 - PathVariable(경로) - 어디로가냐 - 경로에 따라 목적지 달라짐... , ?storeId="
				// + storeId = 결제API에 전달할 data - 뭘 가지고 가냐(바껴도 상관없음)
	...
	public String hotelComplete(HttpSession session, Model model) {
		...
		HotelReserveCompleteDtoRes reserveCompletedto = (HotelReserveCompleteDtoRes) session
				.getAttribute("reserveCompletedto");
